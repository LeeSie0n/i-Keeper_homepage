...
export default function Register() {
  ...
  const [formData, setFormData] = useState({
    ...
    year: "",
    password: "",
    signatureImage: null as File | null, // 서명 파일
  });
  ...
  const sendEmailAuth = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/auth/send-verification-code`, {
        method: "POST",
  ...
  function isValidPassword(password: string) {
    const lengthOk = password.length >= 8 && password.length <= 100;
    const hasLower = /[a-z]/.test(password); 
    const hasUpper = /[A-Z]/.test(password); 
    const hasSpecial = /[!@#$%^&*(),.?":{}|<>]/.test(password); 

    return lengthOk && hasLower && hasUpper && hasSpecial;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    alert(
      "비밀번호는 8자 이상, 영문 대소문자, 특수문자를 포함해야 합니다."
    );
    return;
  }
  ...
  return (
    <section className={`site-container ${styles.auth}`}>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <input name="studentId" placeholder="학번 (숫자 8자)" required onChange={handleChange} />
        <input name="year" placeholder="학년/학차 (0/0)" required onChange={handleChange} />
        <input name="major" placeholder="전공" required onChange={handleChange} />
        <input type="email" name="email" placeholder="이메일 (yourEmail@email.com)" required onChange={handleChange} />
        ...
        <input
          type="password"
          name="password"
          placeholder="비밀번호 (대소문자, 숫자, 특수문자 포함 8자 이상)"
          required
          onChange={handleChange}
        />
        <p>사인 파일 업로드 (PNG) (흰색 바탕 검은색 펜 정자로 사인)</p>
        <input
          type="file"
          name="file"
          accept="image/png"
